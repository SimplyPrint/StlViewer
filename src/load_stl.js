var worker_fn = function () {
    ...
    {
        self.postMessage({msg_type:MSG_ERROR, data:s});
    }
    ...
    function download_from_local_xhr(filename)
    {
        ...
        if (get_progress)
        {
            ...
            function(e)
            {
                self.postMessage({msg_type:MSG_LOAD_IN_PROGRESS, id:model_id, loaded:e.loaded, total:e.total});
            }
        }	
    ...
    async function download_from_local_fetch(filename)
    {
        ...
        while (true)
        {
            ...
            if (get_progress)
            {
                self.postMessage({ msg_type: MSG_LOAD_IN_PROGRESS, id: model_id, loaded: position, total: total });
            }
        }
    ...
    */
    ...
    function after_file_load(s)
    {
        ...
        self.postMessage({msg_type:MSG_STL_LOADED, vertices:vf_data.vertices, faces:vf_data.faces, colors:vf_data.colors});
    }
...
var worker_blob = new Blob(["(" + worker_fn.toString() + ")()"], {type: 'application/javascript'});
