...
class StlViewer {
    ...
    calc_volume_and_area(geo, factor) {
        let x1, x2, x3, y1, y2, y3, z1, z2, z3, i1;
        let len = geo.faces.length;
        let totalVolume = 0;
        let totalArea = 0;
        let a, b, c, s;
        for(let i2 = 0; i2 < len; i2++){
            x1 = geo.vertices[geo.faces[i2].a].x * factor;
            y1 = geo.vertices[geo.faces[i2].a].y * factor;
            z1 = geo.vertices[geo.faces[i2].a].z * factor;
            x2 = geo.vertices[geo.faces[i2].b].x * factor;
            y2 = geo.vertices[geo.faces[i2].b].y * factor;
            z2 = geo.vertices[geo.faces[i2].b].z * factor;
            x3 = geo.vertices[geo.faces[i2].c].x * factor;
            y3 = geo.vertices[geo.faces[i2].c].y * factor;
            z3 = geo.vertices[geo.faces[i2].c].z * factor;
            totalVolume += -x3 * y2 * z1 + x2 * y3 * z1 + x3 * y1 * z2 - x1 * y3 * z2 - x2 * y1 * z3 + x1 * y2 * z3;
            a = geo.vertices[geo.faces[i2].a].distanceTo(geo.vertices[geo.faces[i2].b]) * factor;
            b = geo.vertices[geo.faces[i2].b].distanceTo(geo.vertices[geo.faces[i2].c]) * factor;
            c = geo.vertices[geo.faces[i2].c].distanceTo(geo.vertices[geo.faces[i2].a]) * factor;
            s = (a + b + c) / 2;
    ...
    get_stl_bin(model_id) {
        ...
        for(let i1 = 0; i1 < 80; i1++)d.setUint8(i1, s.charCodeAt(i1), true);
    ...
    vf_to_geo(vertices, faces, colors, default_face_color) {
        ...
        for(let i1 = 0; i1 < len; i1++)geo_vertices.push(new _three.Vector3(vertices[i1][0], vertices[i1][1], vertices[i1][2]));
        len = faces.length;
        if (!colors) for(let i2 = 0; i2 < len; i2++)geo_faces.push(new _three.Face3(faces[i2][0], faces[i2][1], faces[i2][2]));
        else for(let i3 = 0; i3 < len; i3++){
            let face = new _three.Face3(faces[i3][0], faces[i3][1], faces[i3][2]);
            if (typeof faces[i3][3] === "undefined") {
            ...
            } else face.color.setRGB(faces[i3][3], faces[i3][4], faces[i3][5]);
            geo_faces.push(face);
        }
    ...
    set_geo_minmax(model) {
        ...
        if (geo.boundingBox) {
            ...
            let i1 = vertices.length;
            while(i1--){
                if (vertices[i1].x < minx) minx = vertices[i1].x;
                if (vertices[i1].y < miny) miny = vertices[i1].y;
                if (vertices[i1].z < minz) minz = vertices[i1].z;
                if (vertices[i1].x > maxx) maxx = vertices[i1].x;
                if (vertices[i1].y > maxy) maxy = vertices[i1].y;
                if (vertices[i1].z > maxz) maxz = vertices[i1].z;
            }
    ...
    load_local_files(files) {
        ...
        if (files.length > 0) {
            ...
            for(let i1 = 0; i1 < len; i1++){
                switch(files[i1].name.split(".").pop()){
                    case "vsj":
                        this.load_vsj(files[i1]);
                        break;
                    case "vsb":
                        this.load_vsb(files[i1]);
                        break;
                    default:
                ...
                if (this.on_model_drop) this.on_model_drop(files[i1].name);
            }
